...
if (location.pathname.endsWith("code.html")) {
  ...
  if (mode === "aider") {
    ...
    fetch(`${BACKEND_URL}/aider-generate/`, {
      ...
      .then((res) => res.json())
      .then((data) => {
        const code = data.generated_code || "# No code returned.";
        textarea.value = code;
...
if (location.pathname.endsWith("result.html")) {
  ...
  document.getElementById("regenerateBtn").onclick = async () => {
    ...
    if (!session_id) return alert("Session ID missing");

    const formData = new FormData();
    formData.append("session_id", session_id);

    localStorage.setItem("mode", "aider");
    localStorage.setItem("codePayload", JSON.stringify({ code: "Regenerating..." }));
    window.location.href = "code.html";

    await fetch(`${BACKEND_URL}/regenerate/`, {
      method: "POST",
      body: formData,
    })
      .then(res => res.json())
      ...
      .catch(() => {
        localStorage.setItem("codePayload", JSON.stringify({ code: "# Regeneration failed" }));
      });
  };
